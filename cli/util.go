...
func InteractiveSend(ctx context.Context, cctx *cli.Context, srv ServicesAPI, proto *api.MessagePrototype) (*types.SignedMessage, error) {
	wallet.PrintMessageInfo(ctx, &proto.Message, srv.WalletAPI())
	fmt.Println()
	var spec *api.MessageSendSpec
	...
	msg, checks, err := srv.PublishMessage(ctx, proto, cctx.Bool("force") || cctx.Bool("force-send"), spec)
	...
	if xerrors.Is(err, ErrCheckFailed) {
		if !cctx.Bool("interactive") {
			...
			msg, _, err = srv.PublishMessage(ctx, proto, true, spec)
		}
	}
	if err != nil {
