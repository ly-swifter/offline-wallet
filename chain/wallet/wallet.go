...
func (w *LocalWallet) WalletEncryptType(ctx context.Context, addr address.Address) (int, error) {
	return 0, nil
}

func (w *LocalWallet) GetWalletKey(ctx context.Context, addr address.Address) (*key.Key, error) {
...
func (w *LocalWallet) GetDefault() (address.Address, error) {
	...
	return address.NewFromBytes(ki.PrivateKey)
}
...
func (w *LocalWallet) SetDefault(a address.Address) error {
	...
	if err := w.keystore.Put(KDefault, types.KeyInfo{
		Type: ki.Type, PrivateKey: a.Bytes(),
	}); err != nil {
		return err
	}
...
func (w *LocalWallet) WalletNew(ctx context.Context, typ types.KeyType) (address.Address, error) {
	...
	if err != nil {
		...
		if err := w.keystore.Put(KDefault, types.KeyInfo{Type: k.Type, PrivateKey: k.Address.Bytes()}); err != nil {
...
func (w *LocalWallet) WalletHas(ctx context.Context, addr address.Address) (bool, error) {
	k, err := w.tryFind(addr)
	if err != nil {
		return false, err
	}
	return k.PrivateKey != nil, nil
}
...
func (w *LocalWallet) SetAuth(ctx context.Context, pass []byte, code []byte) error {
	return nil
}

func (w *LocalWallet) WalletEncrypt(ctx context.Context, addr address.Address, pass, newPass, code, optUrl []byte) error {
	return nil
}
...
func swapTestForMainnetPrefix(addr string) (string, error) {
	aChars := []rune(addr)
	prefixRunes := []rune(address.MainnetPrefix)
	if len(prefixRunes) != 1 {
		return "", xerrors.Errorf("unexpected prefix length: %d", len(prefixRunes))
	}
